...
void radixSortBlockKeysValues(uint4 *key, uint4 *value, uint nbits, uint startbit, __local uint* sMem, __local uint* sVMem, __local uint* numtrue)
{
	int localId = get_local_id(0);
    int localSize = get_local_size(0);
					...
					__local uint* sVMem)
{
	...
	radixSortBlockKeysValues(&key, &value, nbits, startbit, sMem, sVMem, numtrue);
